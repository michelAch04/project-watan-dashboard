...
{
    /**
     * Display public requests dashboard
     */
    public function index()
    {
        ...
        // Get budgets if user is HOR (only public budgets)
        $budgets = null;
        if ($user->hasRole('hor')) {
            $budgets = Budget::notCancelled()
                ...
                ->forRequestType(Budget::TYPE_PUBLIC)
                ->get()
                ->map(function($budget) {
        ...
        return view('public-requests.index', compact('activeCount', 'draftCount', 'completedCount', 'budgets'));
    }
    ...
    public function active(HttpRequest $httpRequest)
    {
        ...
        $query = RequestHeader::with([
            ...
            'currentUser',
            'referenceMember',
            'publicRequest.city.zone',
            'publicRequest.budget'
        ])
            ->active()
            ->forUser($user)
            ->whereHas('publicRequest');
        ...
        return view('public-requests.active', compact('requests', 'availableMonths', 'month', 'year'));
    }
    ...
    public function completed(HttpRequest $httpRequest)
    {
        ...
        return view('public-requests.completed', compact('requests', 'availableMonths', 'month', 'year'));
    }
    ...
    public function drafts()
    {
        ...
        return view('public-requests.drafts', compact('requests'));
    }
    ...
    public function create()
    {
        ...
        return view('public-requests.create', compact('pwMembers'));
    }
    ...
    public function store(HttpRequest $httpRequest)
    {
        ...
        // Get PW member info for auto-fill
        $pwMember = $user->pwMember;
        if (!$pwMember) {
            return response()->json([
                'success' => false,
                'message' => 'Your account is not linked to a PW member. Please contact administrator.'
            ], 400);
        }
        ...
        try {
            ...
            $publicData = [
                ...
                'requester_full_name' => trim("{$pwMember->first_name} {$pwMember->father_name} {$pwMember->last_name}"),
                'requester_phone' => $pwMember->phone,
            ...
            if (!empty($validated['budget_id'])) {
                $publicData['budget_id'] = $validated['budget_id'];
            }
            ...
            if ($user->hasRole('hor') && $validated['action'] == 'publish' && !empty($validated['budget_id'])) {
                ...
                Log::info("Allocating amount: " . $validated['amount'] . " for new public request id: " . $requestHeader->id . " to budget month: " . $readyDate->format('F Y'));
                ...
                $budget->allocateForRequest(
                    ...
                    $requestHeader->id,
                    "Public Request #{$requestHeader->request_number} allocated to " . $readyDate->format('F Y')
                );
    ...
    {
        $request = RequestHeader::with('publicRequest')->findOrFail($id);
        $user = Auth::user();
        ...
        return view('public-requests.edit', compact('request', 'pwMembers'));
    }
    ...
    {
        $requestHeader = RequestHeader::with('publicRequest')->findOrFail($id);
        $user = Auth::user();
        ...
        $requestHeader->publicRequest->update($publicUpdateData);
        ...
        try {
            ...
            if ($user->manager_id && $user->manager_id != $user->id) {
                ...
                InboxNotification::createForUser(
                    ...
                    "{$user->username} has approved public facilities request #{$requestHeader->request_number}. It now requires your approval."
                );
                ...
                return response()->json([
                    ...
                    'redirect' => route('public-requests.active')
                ]);
            } else {
    ...
    public function finalApprove(HttpRequest $httpRequest, $id)
    {
        ...
        try {
            ...
            $requestAmount = $requestHeader->publicRequest->amount;
            ...
            Log::info("Allocating amount: " . $requestAmount . " for public request id: " . $requestHeader->id . " to budget month: " . $readyDate->format('F Y'));
            ...
            // Update public request with budget_id
            $requestHeader->publicRequest->update([
            ...
            InboxNotification::createForUser(
                ...
                "Your public facilities request #{$requestHeader->request_number} has received final approval and is scheduled for {$readyDate->format('M d, Y')}."
            );
            ...
            DB::rollBack();
            Log::error('Error in finalApprove for public request: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function reject(HttpRequest $httpRequest, $id)
    {
        ...
        try {
            ...
            InboxNotification::createForUser(
                ...
                'request_rejected',
                'Request Rejected',
                "{$user->username} has rejected your public facilities request #{$requestHeader->request_number}. Reason: {$validated['rejection_reason']}"
            );
    ...
    public function markReady($id)
    {
        ...
        InboxNotification::createForUser(
            ...
            "Public facilities request #{$requestHeader->request_number} is now ready for collection."
        );
    ...
    public function markCollected($id)
    {
        ...
        if (!$user->can('mark_collected_public')) {
            return response()->json([
                'success' => false,
                'message' => 'Unauthorized'
            ], 403);
        ...
        InboxNotification::createForUser(
            ...
            'request_collected',
            'Request Collected',
            "Public facilities request #{$requestHeader->request_number} has been collected."
        );
    ...
    public function download($id)
    {
        ...
        if (!$user->can('final_approve_public')) {
            abort(403);
        }
        ...
        return view('public-requests.download', compact('request'));
    }

    /**
     * Search cities (AJAX)
     */
    public function searchCities(HttpRequest $httpRequest)
    {
        ...
        $cities = $query->limit(20)->get()->map(function ($city) {
            return [
                'id' => $city->id,
                'name' => $city->name,
                'name_ar' => $city->name_ar,
                'zone_name' => $city->zone ? $city->zone->name : 'N/A',
                'display_text' => $city->name . ($city->zone ? " ({$city->zone->name})" : '')
            ];
        });
    ...
    public function getAmount($id)
    {
        $requestHeader = RequestHeader::with('publicRequest')->findOrFail($id);
        return response()->json([
            'amount' => $requestHeader->publicRequest->amount
        ]);
    }
    ...
    public function exportMonthlyPDF(HttpRequest $httpRequest)
    {
        ...
        return view('public-requests.download', compact('requests', 'month', 'year'));
    }
