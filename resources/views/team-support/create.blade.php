...
@section('title', 'Create Team Support Request')

@section('content')
<div class="min-h-screen bg-[#fcf7f8]" x-data="teamSupportForm(@json(auth()->user()->hasRole('hor')))" x-init="init()">
    {{-- ... (Header is unchanged) ... --}}
            ...
            <div class="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-[#f8f0e2]">
                <form @submit.prevent="submitForm" class="space-y-8">
                    ...
                    <div class="bg-[#f8f0e2] p-4 rounded-lg border-2 border-[#931335]/20">
                        ...
                        {{-- ... (Selected PW Member display is unchanged and correct) ... --}}
...
<script>
    ...
    function teamSupportForm(isHor) {
        return {
            form: {
                voter_id: '',
                requester_city_id: '',
            ...
            selectPwMember(pwMember) {
                ...
                this.form.requester_city_id = pwMember.city_id;
            ...
            async submitForm() {
                ...
                try {
                    ...
                    const response = await fetch('{{ route("team-support.store") }}', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'Accept': 'application/json'
