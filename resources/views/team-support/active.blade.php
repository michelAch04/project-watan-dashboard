        ...
        <div class="safe-area">
            <div class="page-container py-4">
                <div class="flex items-center">
                    <a href="{{ route('team-support.index') }}" class="p-2 hover:bg-[#f8f0e2] rounded-lg transition-all mr-2">
            ...
            <div class="bg-white rounded-xl p-4 shadow-sm border border-[#f8f0e2]">
                <div class="space-y-3">
                    <div class="w-full">
                        ...
                        <form method="GET" action="{{ route('team-support.active') }}" class="flex flex-row gap-2">
                            <input type="month"
                            ...
                            <div class="flex items-center gap-2">
                                @if($month && $year)
                                <a href="{{ route('team-support.active') }}" class="flex-1 sm:flex-initial btn-secondary whitespace-nowrap">Clear</a>
                                @endif
                            </div>
                        </form>
                    </div>
                    ...
                    @if($month && $year)
                    <div class="w-full">
                        <a href="{{ route('team-support.export-active-pdf', ['month' => $month, 'year' => $year]) }}"
                ...
                <div class="bg-white rounded-xl p-4 shadow-sm border border-[#f8f0e2]"
                    ...
                    <div class="mb-3 pb-3 border-b border-[#f8f0e2]">
                        <div class="flex items-start gap-2 mb-2">
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-semibold text-[#622032]" lang="ar">{{ $request->teamSupportRequest->requester_full_name }}</p>
                                <p class="text-xs text-[#622032]/60" lang="ar">{{ $request->teamSupportRequest->pwMember->voter->city->name }} @if($request->teamSupportRequest->pwMember->register_number) • {{ $request->teamSupportRequest->pwMember->register_number }} @endif</p>
                            </div>
                        </div>

                        <div class="flex items-center gap-2 text-xs text-[#622032]/60">
                            <span class="px-2 py-1 bg-[#fef9de] rounded">{{ $request->teamSupportRequest->subtype }}</span>
                            <span>•</span>
                            <span class="font-semibold text-[#931335]">${{ number_format($request->teamSupportRequest->amount, 2) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 space-y-2 text-xs">
                        ...
                        @endif
                        @if($request->teamSupportRequest->budget)
                        <div class="flex items-center gap-2">
                            <span class="text-[#622032]/60">Budget:</span>
                            <span class="font-semibold text-[#622032]">{{ $request->teamSupportRequest->budget->description }}</span>
                        </div>
                        @endif
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 pt-3 border-t border-[#f8f0e2]">
                        <a href="{{ route('team-support.show', $request->id) }}"
    ...
    function activeRequests() {
        return {
            ...
            async confirmApprove() {
                ...
                try {
                    const response = await fetch(`/team-support/${this.selectedRequestId}/approve`, {
                        method: 'POST',
                        headers: {
            ...
            async showBudgetSelectionModal() {
                try {
                    ...
                    if (response.ok && data.success) {
                        ...
                        // Get request amount
                        const requestResponse = await fetch(`/team-support/${this.selectedRequestId}/amount`, {
                            headers: {
                                'Accept': 'application/json',
                                'X-CSRF-TOKEN': csrfToken
                            }
            ...
            async confirmFinalApprove() {
                this.processing = true;
                try {
                    const response = await fetch(`/team-support/${this.selectedRequestId}/final-approve`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            async confirmReject() {
                this.processing = true;
                try {
                    const response = await fetch(`/team-support/${this.selectedRequestId}/reject`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            async markReady(id) {
                ...
                try {
                    const response = await fetch(`/team-support/${id}/mark-ready`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'Accept': 'application/json'
            ...
            async markCollected(id) {
                ...
                try {
                    const response = await fetch(`/team-support/${id}/mark-collected`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'Accept': 'application/json'
