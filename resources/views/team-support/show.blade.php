        ...
        <div class="safe-area">
            <div class="page-container py-4">
                <div class="flex items-center">
                    <a href="{{ route('team-support.active') }}" class="p-2 hover:bg-[#f8f0e2] rounded-lg transition-all mr-2">
    ...
    <div class="safe-area py-4">
        <div class="page-container space-y-4">
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-[#f8f0e2]">
                ...
                <!-- Warning: PW Member has rejected requests -->
                ...
                <div class="mt-4 p-4 bg-amber-50 border-l-4 border-amber-500 rounded-lg">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-amber-700 mb-2">This pwMember has {{ $rejectedRequests->count() }} rejected request(s):</p>
                            <ul class="text-xs text-amber-700 space-y-1">
                                @foreach($rejectedRequests as $rejReq)
                                <li class="flex items-center gap-2">
                                    <span class="w-1.5 h-1.5 bg-amber-600 rounded-full"></span>
                                    <a href="{{ route('team-support.show', $rejReq->requestHeader->id) }}" class="hover:underline font-semibold">
                ...
                <div class="space-y-3">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-sm font-semibold text-[#622032]">{{ $request->teamSupportRequest->requester_full_name }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-[#622032]/60 mb-1">City (البلدة)</p>
                            <p class="text-sm font-semibold text-[#622032]">{{ $request->teamSupportRequest->pwMember->voter->city->name }} - {{ $request->teamSupportRequest->pwMember->voter->city->name_ar }}</p>
                        </div>
                    </div>

                    @if($request->teamSupportRequest->pwMember->voter->ro_number)
                    <div>
                        <p class="text-xs text-[#622032]/60 mb-1">رقم السجل</p>
                        <p class="text-sm font-semibold text-[#622032]">{{ $request->teamSupportRequest->pwMember->voter->ro_number }}</p>
                    </div>
                    @endif

                    @if($request->teamSupportRequest->pwMember->phone)
                    <div>
                        <p class="text-xs text-[#622032]/60 mb-1">Phone Number</p>
                        <p class="text-sm font-semibold text-[#622032]">{{ $request->teamSupportRequest->pwMember->phone }}</p>
                    </div>
                    @endif

                    @if($request->teamSupportRequest->pwMember)
                    <div class="p-3 bg-[#fef9de] rounded-lg">
                        <p class="text-xs text-[#622032]/60 mb-1">Linked to PW Member</p>
                        <p class="text-sm font-semibold text-[#931335]">{{ $request->teamSupportRequest->pwMember->full_name }}</p>
                    </div>
                    @endif
                </div>
            </div>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-sm font-semibold text-[#622032]">{{ $request->teamSupportRequest->subtype }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-[#622032]/60 mb-1">Amount</p>
                            <p class="text-2xl font-bold text-[#931335]">${{ number_format($request->teamSupportRequest->amount, 2) }}</p>
                        </div>
                    </div>
                    ...
                    @if($request->teamSupportRequest->notes)
                    <div>
                        <p class="text-xs text-[#622032]/60 mb-1">Notes</p>
                        <div class="p-3 bg-[#fcf7f8] rounded-lg">
                            <p class="text-sm text-[#622032]">{{ $request->teamSupportRequest->notes }}</p>
                        </div>
                    </div>
                    @endif
                </div>
            ...
            @if($request->teamSupportRequest->supporting_documents)
            <div class="bg-white rounded-xl p-6 shadow-sm border border-[#f8f0e2]">
                <x-supporting-documents
                    :documents="$request->teamSupportRequest->supporting_documents"
            ...
            <div class="space-y-3">
                ...
                <a href="{{ route('team-support.edit', $request->id) }}"
                ...
                <div class="grid @if($request->requestStatus->name !== 'final_approval' && $request->requestStatus->name !== 'ready_for_collection')
                    ...
                    <a href="{{ route('team-support.download', $request->id) }}"
    ...
    function requestDetails() {
        return {
            ...
            async confirmApprove() {
                ...
                try {
                    const response = await fetch('{{ route("team-support.approve", $request->id) }}', {
                        method: 'POST',
                        headers: {
            ...
            async showBudgetSelectionModal() {
                try {
                    ...
                    const response = await fetch('/api/budgets/my-zones?request_type=team-support', {
                        headers: {
                            'Accept': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
                        }
                    ...
                    if (response.ok && data.success) {
                        ...
                        // Get request amount
                        const requestResponse = await fetch(`/team-support/{{ $request->id }}/amount`, {
                            headers: {
                                'Accept': 'application/json',
                                'X-CSRF-TOKEN': csrfToken
                            }
            ...
            async confirmFinalApprove() {
                this.processing = true;
                try {
                    const response = await fetch(`/team-support/{{ $request->id }}/final-approve`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            async confirmReject() {
                this.processing = true;
                try {
                    const response = await fetch('{{ route("team-support.reject", $request->id) }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
            ...
            async markReady() {
                ...
                try {
                    const response = await fetch('{{ route("team-support.mark-ready", $request->id) }}', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'Accept': 'application/json'
            ...
            async markCollected() {
                ...
                try {
                    const response = await fetch('{{ route("team-support.mark-collected", $request->id) }}', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'Accept': 'application/json'
