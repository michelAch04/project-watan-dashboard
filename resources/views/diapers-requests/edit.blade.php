...
@section('title', 'Edit Diapers Request')

@section('content')
@php
$voterData = $request->diapersRequest->voter ? [
'id' => $request->diapersRequest->voter->id,
'full_name' => $request->diapersRequest->voter->full_name,
'city_name' => $request->diapersRequest->voter->city->name,
'register_number' => $request->diapersRequest->voter->register_number,
'city_id' => $request->diapersRequest->voter->city_id
] : null;
...
@endphp
<div class="min-h-screen bg-[#fcf7f8]" x-data="diapersEditForm(@json(auth()->user()->hasRole('hor')))" x-init="init()">
    <div class="mobile-header">
        <div class="safe-area">
            <div class="page-container py-4">
                <div class="flex items-center">
                    <a href="{{ route('diapers-requests.drafts') }}" class="p-2 hover:bg-[#f8f0e2] rounded-lg transition-all mr-2">
                        <svg class="w-5 h-5 text-[#622032]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                        </svg>
                    </a>
                    <h1 class="text-lg sm:text-xl font-bold text-[#622032]">Edit Diapers Request</h1>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function diapersEditForm(isHor) {
        return {
            isHor: isHor,
            form: {
                voter_id: '{{ $request->diapersRequest->voter_id ?? "" }}',
                requester_city_id: '{{ $request->diapersRequest->requester_city_id }}',
                reference_member_id: '{{ $request->reference_member_id }}',
                items: @json($request->diapersRequest->items ?? [['size' => '', 'count' => '']]),
                notes: `{{ $request->diapersRequest->notes }}`,
                action: 'save',
                diaper_budget_id: '{{ $request->diaper_budget_id ?? "" }}',
            ...
            voterSearch: '{{ $request->diapersRequest->voter ? $request->diapersRequest->voter->full_name : "" }}',
            ...
            async submitForm() {
                ...
                try {
                    const response = await fetch('{{ route("diapers-requests.update", $request->id) }}', {
                        method: 'PUT',
                        headers: {
