            ...
            <div class="page-container py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <a href="javascript:history.back()" class="p-2 hover:bg-[#f8f0e2] rounded-lg transition-all mr-2">
    ...
    <div class="safe-area py-4">
        <div class="page-container space-y-4">
            ...
            <div class="space-y-3">
                ...
                @php
                    $requestType = $item->requestHeader->getRequestType();
                    $specificRequest = $item->requestHeader->getSpecificRequest();
                @endphp
                <div class="bg-white rounded-xl p-4 shadow-sm border border-[#f8f0e2]">
                    ...
                    <div class="flex items-start justify-between mb-3">
                        <div>
                            ...
                            <p class="text-xs text-[#622032]/60">
                                <span class="px-2 py-0.5 bg-purple-100 text-purple-700 rounded">{{ ucfirst($requestType) }}</span>
                                Template Request
                            </p>
                        </div>
                        ...
                        <div class="flex items-start gap-2 mb-2">
                            ...
                            <div class="flex-1">
                                @if($requestType === 'humanitarian')
                                    <p class="text-sm font-semibold text-[#622032]" lang="ar">{{ $specificRequest->requester_full_name }}</p>
                                    <p class="text-xs text-[#622032]/60" lang="ar">{{ $specificRequest->voter->city->name }}</p>
                                @elseif($requestType === 'public')
                                    <p class="text-sm font-semibold text-[#622032]" lang="ar">{{ $specificRequest->requester_full_name }}</p>
                                    <p class="text-xs text-[#622032]/60" lang="ar">{{ $specificRequest->city->name }}</p>
                                @elseif($requestType === 'diapers')
                                    <p class="text-sm font-semibold text-[#622032]" lang="ar">{{ $specificRequest->voter->first_name }} {{ $specificRequest->voter->father_name }} {{ $specificRequest->voter->last_name }}</p>
                                    <p class="text-xs text-[#622032]/60" lang="ar">{{ $specificRequest->voter->city->name }}</p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end">
                        ...
                        @elseif($requestType === 'public')
                            <a href="{{ route('public-requests.show', $item->requestHeader->id) }}" class="text-sm text-[#931335] hover:underline">
                                View Original Request
                            </a>
                        @elseif($requestType === 'diapers')
